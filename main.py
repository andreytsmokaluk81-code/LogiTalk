...
class MainWindow(CTk):
    def __init__(self):
        ...
        try:
            ...
            hello = "TEXT@{self.username}@[SYSTEM] {self.username] приєднався"
            self.sock.send(hello.encode())
            threading.Thread(target = self.recv.message, demon = True).start()
        except Exception as e:
            print(e)
    def add_message(self, message):
        message_frame = CTkFrame(self.chat_field, fg_color='blue')
        message_frame.pack(pady=5, anchor='w')
        CTkLabel(message_frame, text=message, text_color='white', justify='left').pack(pady=10, pady=5)
    ...
    def toggle_menu(self):
        if self.is_show_menu:
            ...
            self.btn.configure(text='<')
            self.show_menu()
            self.label = CTkLabel(self.menu_frame, text='Введіть налаштування')
